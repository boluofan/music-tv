...
public class PlaylistDetailActivity extends AppCompatActivity {
    ...
    private void loadMiMusicPlaylistDetail() {
        int playlistIdInt;
        try {
            playlistIdInt = Integer.parseInt(playlistId);
        ...
        // 先获取歌单详情
        apiService.getMiMusicPlaylistDetail(playlistIdInt).enqueue(new Callback<MiPlaylist>() {
            @Override
            public void onResponse(Call<MiPlaylist> call, Response<MiPlaylist> response) {
                if (!response.isSuccessful() || response.body() == null) {
                    showLoading(false);
                    Toast.makeText(PlaylistDetailActivity.this, "加载失败", Toast.LENGTH_SHORT).show();
                    return;
                ...
                MiPlaylist miPlaylist = response.body();
                // 再获取歌曲列表
                apiService.getMiMusicPlaylistSongs(playlistIdInt, 500, 0).enqueue(new Callback<MiPlaylistSongsResponse>() {
                    @Override
                    public void onResponse(Call<MiPlaylistSongsResponse> call, Response<MiPlaylistSongsResponse> response) {
                        showLoading(false);
                        if (!response.isSuccessful() || response.body() == null) {
                            Toast.makeText(PlaylistDetailActivity.this, "加载歌曲失败", Toast.LENGTH_SHORT).show();
                            return;
                        }

                        // 将 MiPlaylist 转换为 Playlist
                        Playlist playlist = miPlaylist.toPlaylist();
                        ...
                        if (miSongs != null) {
                            for (MiSong miSong : miSongs) {
                                musicInfoList.add(miSong.toMusicInfo());
                            }
                        }
                        playlist.setSongs(musicInfoList);
                        playlist.setSongCount(musicInfoList.size());
            ...
            @Override
            public void onFailure(Call<MiPlaylist> call, Throwable t) {
                showLoading(false);
                Toast.makeText(PlaylistDetailActivity.this, "网络错误: " + t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
